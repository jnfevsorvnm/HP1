...
client.on('message', message => { 
    ...
    if (!message.guild.member(user).roleable) return message.reply("**لايمكنني اعطاء شخص اعلى من رتبتي يرجه اعطاء البوت رتبه عالي**").then(msg => {msg.delete(5000)});
    message.guild.member(user).addRole(message.guild.roles.find("name", MRole));
    message.reply('***تم اعطاء الشخص رتبة بنجاح:white_check_mark:***').then(msg => {msg.delete(10000)});
    }
    });
