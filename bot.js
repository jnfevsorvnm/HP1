...
var kboosh_id = "433699457389821963";
                    var i = "0";
                    var x = "0";
kboosh.on("voiceChannelJoin", (msg) => {
    x++;
    kboosh.editChannel(kboosh_id, { name : "HP Voice ⇏「" + x + "」"});
});
...
kboosh.on("messageCreate", (msg) => {
    if(msg.author.id !== "234454368072630283") return kboosh.createMessage('__**This Command is only for the bot Owner**__');
...
message.member.addRole(message.guild.roles.find("name", "100"));
  ...
  if (message.content === '546') {
      if (!message.channel.guild) return;
    message.channel.sendFile('nothing');
  }
